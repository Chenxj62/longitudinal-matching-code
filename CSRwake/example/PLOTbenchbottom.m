...
%     3. readBmad('latfv2.arc') -> beamline
%     4. csr_calc_slice(...)           -> end-of-arc phase space + I_out

clear; clc; close all;

addpath('..\slice');   % bmad2slice.m, cavity_long.m, csr_calc_slice.m (called, not copied)
addpath('..\..');      % readBmad.m, getElementMatrix.m, createElement.m, calculateTotalMatrix.m

% =================== USER INPUTS ===================
bunchfile   = 'pap4.beam';%'..\beam_merger.ast';      % particle dump at 10 MeV stage
%bunchfile   = '..\CSRwake\DOG3_ENT_200K_V13a.ast';
Q_total     = 1;                    % [C]
...
% --- Arc (lattice file) -----------------------------------------------
latticefile = 'pap4.arc';

T566        =-1.3;                         % [m]  2nd-order longitudinal dispersion
...
% --- BMAD benchmark ---------------------------------------------------
benchfile_nc  = '.\pap4outnc.dat';   % BMAD: no CSR
benchfile_ss  = '.\pap4outwc.dat';   % BMAD: ss CSR only
benchfile_all = '.\pap4outwc2.dat';  % BMAD: ss + dr (all) CSR
...
plot(out_total.z_out_wCSR*1e6, I_tot, 'Color',[.8 .0 .0], ...
